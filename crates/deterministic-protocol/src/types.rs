...
pub struct PatchApplyParams {
    ...
    /// When present, these edits are from a hybrid worker's approved proposals.
    /// The value is the `worker_run_id` that produced them.
    /// The daemon validates that only the approved patches are applied.
    #[serde(default)]
    pub hybrid_worker_run_id: Option<String>,
    /// When true, the approval policy check is skipped.
    /// Used when ChatGPT re-calls patch.apply after approve_action resolved
    /// a previously pending approval for hybrid worker proposals.
    #[serde(default)]
...
mod harness_mode_tests {
    ...
    fn parse_accepts_deterministic() {
        assert_eq!(
            HarnessMode::parse("deterministic"),
            Some(HarnessMode::Deterministic)
        );
    }
...
pub struct HybridWorkerRun {
    ...
    #[serde(default)]
    pub focus_paths: Vec<String>,
    /// Context files specified at prepare time.  Loaded from disk at start time.
    #[serde(default)]
...
/// Parameters for submitting a hybrid worker's proposed patches for approval.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct HybridPatchSubmitParams {
    /// Parent run ID (the run that owns the worker).
    pub run_id: String,
    /// Worker run ID whose proposed edits should be submitted.
    pub worker_run_id: String,
    /// Zero-based indices into the worker run's `proposed_edits` array.
    /// Only the patches at these indices will be submitted for approval.
    pub patch_indices: Vec<usize>,
}

/// Result of submitting hybrid worker patches for approval.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct HybridPatchSubmitResult {
    /// IDs of the pending approvals created, in submission order.
    pub approval_ids: Vec<String>,
    /// How many patches were submitted.
    pub submitted_count: usize,
    /// Summary of the submitted patches.
    pub summary: String,
