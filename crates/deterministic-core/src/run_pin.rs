...
use deterministic_protocol::{
    PIN_REASON_MAX_LEN, PinMetadata, RunPinParams, RunPinResult, RunState,
};
...
pub fn pin(params: &RunPinParams, state: &mut RunState) -> Result<RunPinResult> {
    ...
    if reason.len() > PIN_REASON_MAX_LEN {
        bail!("pin reason exceeds maximum length of {PIN_REASON_MAX_LEN} characters");
    }
...
mod tests {
    ...
    fn pin_active_run_succeeds() {
        ...
        assert_eq!(
            state.pin_metadata.as_ref().unwrap().reason,
            "primary effort"
        );
    }
