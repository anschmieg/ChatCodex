...
pub fn archive(params: &RunArchiveParams, state: &mut RunState) -> Result<RunArchiveResult> {
...
mod tests {
    ...
    fn archive_completed_run_succeeds() {
        ...
        let meta = state
            .archive_metadata
            .as_ref()
            .expect("archive_metadata must be set");
    ...
    fn archive_already_archived_run_rejected() {
        ...
        assert_eq!(
            state.archive_metadata.as_ref().unwrap().reason,
            "first archive"
        );
    }
    ...
    fn archive_does_not_change_status() {
        for outcome_kind in &["completed", "failed", "abandoned"] {
            ...
            assert_eq!(
                ...
                "status must not change after archiving"
            );
        }
    }
