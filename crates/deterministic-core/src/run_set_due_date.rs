...
use deterministic_protocol::types::{RunSetDueDateParams, RunSetDueDateResult, RunState};
...
pub fn validate_due_date(s: &str) -> Result<String> {
    let trimmed = s.trim();
    if trimmed.len() != 10 {
        bail!("due_date must be exactly 10 characters in YYYY-MM-DD format, got: '{trimmed}'");
    }
