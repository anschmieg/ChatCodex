...
use deterministic_protocol::types::{
    MAX_DEPENDENCY_COUNT, RunSetDependenciesParams, RunSetDependenciesResult, RunState,
};
...
mod tests {
    ...
    fn set_multiple_dependencies_sorted() {
        ...
        let result =
            set_dependencies(&params, &mut state, &known(&["run-b", "run-m", "run-z"])).unwrap();
