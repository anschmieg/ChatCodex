...
use deterministic_protocol::{
    LABEL_MAX_COUNT, LABEL_MAX_LEN, OPERATOR_NOTE_MAX_LEN, RunAnnotateParams, RunAnnotateResult,
    RunState,
};
...
fn normalize_label(raw: &str) -> Result<String> {
    ...
    if trimmed.len() > LABEL_MAX_LEN {
        bail!("label '{trimmed}' exceeds maximum length of {LABEL_MAX_LEN} characters");
    }
...
pub fn annotate(params: &RunAnnotateParams, state: &mut RunState) -> Result<RunAnnotateResult> {
    ...
    if params.labels.is_none() && params.operator_note.is_none() {
        bail!("run.annotate requires at least one of 'labels' or 'operatorNote'");
    }
    ...
    if let Some(note) = &params.operator_note {
        if note.len() > OPERATOR_NOTE_MAX_LEN {
            bail!("operator note exceeds maximum length of {OPERATOR_NOTE_MAX_LEN} characters");
        }
...
mod tests {
    ...
    fn annotate_sets_labels() {
        ...
        assert_eq!(
            state.annotation.as_ref().unwrap().labels,
            vec!["auth", "infra"]
        );
    }
