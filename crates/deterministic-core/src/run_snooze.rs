...
use deterministic_protocol::{
    RunSnoozeParams, RunSnoozeResult, RunState, SNOOZE_REASON_MAX_LEN, SnoozeMetadata,
};
...
pub fn snooze(params: &RunSnoozeParams, state: &mut RunState) -> Result<RunSnoozeResult> {
    ...
    if reason.len() > SNOOZE_REASON_MAX_LEN {
        bail!("snooze reason exceeds maximum length of {SNOOZE_REASON_MAX_LEN} characters");
    }
