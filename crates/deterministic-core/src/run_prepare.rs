...
use deterministic_protocol::{
    HarnessMode, RunPolicy, RunPrepareParams, RunPrepareResult, RunState,
};
use uuid::Uuid;
...
mod tests {
    ...
    fn prepare_uses_default_policy_when_none_provided() {
        ...
        assert_eq!(
            result.effective_policy.patch_edit_threshold,
            defaults.patch_edit_threshold
        );
        assert_eq!(
            result.effective_policy.delete_requires_approval,
            defaults.delete_requires_approval
        );
        assert_eq!(
            state.policy_profile.patch_edit_threshold,
            defaults.patch_edit_threshold
        );
    }
