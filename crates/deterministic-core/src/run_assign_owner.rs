...
use deterministic_protocol::types::{
    ASSIGNEE_MAX_LEN, OWNERSHIP_NOTE_MAX_LEN, RunAssignOwnerParams, RunAssignOwnerResult, RunState,
};
...
pub fn normalize_assignee(s: &str) -> Result<String> {
    ...
    if lower.len() > ASSIGNEE_MAX_LEN {
        bail!("assignee exceeds maximum length of {ASSIGNEE_MAX_LEN} characters");
    }
