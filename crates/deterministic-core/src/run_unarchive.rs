...
pub fn unarchive(params: &RunUnarchiveParams, state: &mut RunState) -> Result<RunUnarchiveResult> {
...
mod tests {
    ...
    fn unarchive_completed_run_succeeds() {
        ...
        let meta = state
            .unarchive_metadata
            .as_ref()
            .expect("unarchive_metadata must be set");
        ...
        let arch = state
            .archive_metadata
            .as_ref()
            .expect("archive_metadata must remain");
    ...
    fn unarchive_already_unarchived_run_rejected() {
        ...
        assert_eq!(
            state.unarchive_metadata.as_ref().unwrap().reason,
            "first unarchive"
        );
    }
    ...
    fn unarchive_does_not_change_status() {
        for outcome_kind in &["completed", "failed", "abandoned"] {
            ...
            assert_eq!(
                ...
                "status must not change after unarchiving"
            );
        }
    }
    ...
    fn unarchive_does_not_reopen_run() {
        ...
        assert!(
            state.status.starts_with("finalized:"),
            "unarchiving must not reopen the run"
        );
