...
use deterministic_protocol::{
    PRIORITY_REASON_MAX_LEN, RunSetPriorityParams, RunSetPriorityResult, RunState,
};
...
pub fn set_priority(
    ...
    if reason.len() > PRIORITY_REASON_MAX_LEN {
        bail!("priority reason exceeds maximum length of {PRIORITY_REASON_MAX_LEN} characters");
    }
...
mod tests {
    ...
    use deterministic_protocol::{PinMetadata, RunOutcome, RunPolicy, RunPriority, SnoozeMetadata};
