...
pub fn run(params: &TestsRunParams, workspace_root: &str) -> Result<TestsRunResult> {
    ...
    anyhow::ensure!(!params.scope.is_empty(), "scope must not be empty");
    anyhow::ensure!(
...
mod tests {
    ...
    fn reject_empty_scope() {
        ...
        assert!(
            err.contains("scope must not be empty"),
    ...
    fn reject_empty_reason() {
        ...
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("reason must not be empty"),
            "error should mention empty reason: {err}"
        );
    }
    ...
    fn reject_unsupported_scope() {
        ...
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("unsupported test scope"),
            "error should mention unsupported scope: {err}"
        );
    }
