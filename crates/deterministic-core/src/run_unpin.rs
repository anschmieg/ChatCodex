...
use deterministic_protocol::{PIN_REASON_MAX_LEN, RunState, RunUnpinParams, RunUnpinResult};
...
pub fn unpin(params: &RunUnpinParams, state: &mut RunState) -> Result<RunUnpinResult> {
    ...
    if reason.len() > PIN_REASON_MAX_LEN {
        bail!("unpin reason exceeds maximum length of {PIN_REASON_MAX_LEN} characters");
    }
