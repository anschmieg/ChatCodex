...
use deterministic_protocol::{
    ArchiveMetadata, HarnessMode, HybridWorkerStatus, PendingApproval, PinMetadata, ReopenMetadata,
    RetryableAction, RunAnnotation, RunEffort, RunHistoryEntry, RunOutcome, RunPolicy, RunPriority,
    RunState, RunSummary, SnoozeMetadata, UnarchiveMetadata,
};
...
impl Store {
    ...
    fn migrate(conn: &Connection) -> Result<()> {
        ...
        // Phase 5 / 7.1: hybrid worker runs table.
        // Must be created BEFORE the column migrations loop runs since migrations
        // include ADD COLUMN for this table.
        conn.execute_batch(
        ...
        let migrations = [
            ...
            // Phase 7.1: hybrid worker context files
            (
                "hybrid_worker_runs",
                "context_files",
    ...
    pub fn save_run(&self, state: &RunState) -> Result<()> {
        ...
        let plan_json = serde_json::to_string(&state.plan).context("failed to serialise plan")?;
        ...
        let focus_paths_json =
            serde_json::to_string(&state.focus_paths).context("failed to serialise focus_paths")?;
        let warnings_json =
        ...
        let is_archived: i64 =
            if state.archive_metadata.is_some() && state.unarchive_metadata.is_none() {
                1
            } else {
                0
            };
        ...
        let is_snoozed: i64 = if state.snooze_metadata.is_some() {
            1
        } else {
            0
        };
    ...
    pub fn get_run(&self, run_id: &str) -> Result<Option<RunState>> {
        ...
        let mut rows = stmt
            .query_map(rusqlite::params![run_id], |row| {
                ...
                let plan: Vec<String> = serde_json::from_str(&plan_json).map_err(|e| {
                ...
                let warnings: Vec<String> = serde_json::from_str(&warnings_json).map_err(|e| {
                    rusqlite::Error::FromSqlConversionFailure(
                        15,
                        rusqlite::types::Type::Text,
                        Box::new(e),
                ...
                let blocked_by_run_ids_json: String =
                    row.get(33).unwrap_or_else(|_| "[]".to_string());
                ...
                let harness_mode =
                    HarnessMode::parse(&harness_mode_str).unwrap_or(HarnessMode::Deterministic);
        ...
        let status = match decision {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "invalid decision: must be 'approve' or 'deny'"
                ));
            }
        };
        let affected = conn
    ...
    pub fn list_runs(
        ...
        let mapper = |row: &rusqlite::Row<'_>| -> rusqlite::Result<RunSummary> {
            ...
            let is_archived = if is_archived_int != 0 {
                Some(true)
            } else {
                None
            };
        ...
        let rows = match (workspace_idx, status_idx) {
            ...
            (Some(_), Some(_)) => stmt
                .query_map(
                    rusqlite::params![limit as i64, workspace_id, status_filter],
                    mapper,
                )
                .context("failed to query runs")?,
        };
    ...
    pub fn get_audit_entries(&self, run_id: &str, limit: usize) -> Result<Vec<RunHistoryEntry>> {
        let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("{e}"))?;
        let mut stmt = conn
            .prepare(
    ...
    pub fn save_worker_run(&self, run: &deterministic_protocol::HybridWorkerRun) -> Result<()> {
        ...
        let context_files_json = serde_json::to_string(&run.context_files)
            .context("failed to serialise worker context_files")?;
        ...
        conn.execute(
            "INSERT OR REPLACE INTO hybrid_worker_runs
                (worker_run_id, parent_run_id, status, provider_profile_id,
                 task_goal, focus_paths, context_files, prompt, proposed_edits, summary,
    ...
    pub fn get_worker_run(
        &self,
        worker_run_id: &str,
    ) -> Result<Option<deterministic_protocol::HybridWorkerRun>> {
        let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("{e}"))?;
        let mut stmt = conn
            .prepare(
    ...
    pub fn list_worker_runs(
        ...
        if let Some(status) = status_filter {
            let mut stmt = conn
                ...
                .context("failed to prepare statement")?;
            let rows = stmt
                .query_map(rusqlite::params![parent_run_id, status.as_str()], |row| {
                    Self::map_worker_row(row)
                })?;
            for row in rows {
    ...
    /// Count worker runs with status `Running`.
    ///
    /// If `parent_run_id` is `None`, counts all running workers globally.
    /// If `parent_run_id` is `Some`, counts only workers for that parent.
    pub fn count_running_workers(
        &self,
        parent_run_id: Option<&str>,
    ) -> Result<usize> {
        let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("{e}"))?;
        let count: i64 = if let Some(pid) = parent_run_id {
            conn.query_row(
                "SELECT COUNT(*) FROM hybrid_worker_runs WHERE status = ?1 AND parent_run_id = ?2",
                rusqlite::params![HybridWorkerStatus::Running.as_str(), pid],
                |row| row.get(0),
            )
        } else {
            conn.query_row(
                "SELECT COUNT(*) FROM hybrid_worker_runs WHERE status = ?1",
                rusqlite::params![HybridWorkerStatus::Running.as_str()],
                |row| row.get(0),
            )
        }
        .map_err(|e| anyhow::anyhow!("failed to count running workers: {e}"))?;
        Ok(count as usize)
    }

    fn map_worker_row(
        row: &rusqlite::Row<'_>,
    ) -> rusqlite::Result<deterministic_protocol::HybridWorkerRun> {
        use deterministic_protocol::{
            HybridWorkerContextFile, HybridWorkerRun, HybridWorkerStatus, PatchEdit,
        };
        let focus_paths_json: String = row.get(5)?;
        let focus_paths: Vec<String> = serde_json::from_str(&focus_paths_json).unwrap_or_default();
        let context_files_json: String = row.get(6)?;
        let context_files: Vec<HybridWorkerContextFile> =
            serde_json::from_str(&context_files_json).unwrap_or_default();
        let proposed_edits_json: Option<String> = row.get(8)?;
        ...
        let status = HybridWorkerStatus::parse(&status_str).unwrap_or(HybridWorkerStatus::Prepared);
        let cancel_requested: i64 = row.get(15)?;
        Ok(HybridWorkerRun {
...
mod tests {
    ...
    fn roundtrip_in_memory() {
        ...
        assert_eq!(loaded.recommended_next_action.as_deref(), Some("inspect"));
    }
    ...
    fn upsert_updates_existing() {
        ...
        assert_eq!(loaded.latest_test_result.as_deref(), Some("all passed"));
    ...
    fn approval_resolve() {
        ...
        store
            .resolve_approval("a1", "approve", Some("LGTM"))
            .unwrap();
        let pending = store.get_pending_approvals("r1").unwrap();
    ...
    fn approval_deny() {
        ...
        store
            .resolve_approval("a1", "deny", Some("too risky"))
            .unwrap();
        let resolved = store.get_approval("a1").unwrap().unwrap();
    ...
    fn list_runs_empty() {
        let store = Store::open_in_memory().unwrap();
        let runs = store
            .list_runs(20, None, None, false, false, None, false, false, false)
            .unwrap();
    ...
    fn list_runs_respects_limit() {
        ...
        let runs = store
            .list_runs(3, None, None, false, false, None, false, false, false)
            .unwrap();
        assert_eq!(runs.len(), 3);
    }
    ...
    fn fresh_database_has_audit_trail_table() {
        let store = Store::open_in_memory().unwrap();
        store
            .save_run(&make_run_state("r_fresh", "active"))
            .unwrap();
    ...
    fn policy_profile_default_roundtrip() {
        ...
        assert_eq!(
            loaded.policy_profile.patch_edit_threshold,
            defaults.patch_edit_threshold
        );
        assert_eq!(
            loaded.policy_profile.delete_requires_approval,
            defaults.delete_requires_approval
        );
        assert_eq!(
            loaded.policy_profile.sensitive_path_requires_approval,
            defaults.sensitive_path_requires_approval
        );
    ...
    fn list_runs_includes_reopen_count() {
        ...
        let summaries = store
            .list_runs(10, None, None, false, false, None, false, false, false)
            .unwrap();
    ...
    fn supersession_lineage_roundtrip() {
        ...
        assert_eq!(
            loaded_orig.superseded_by_run_id.as_deref(),
            Some("r_successor")
        );
        assert_eq!(
            loaded_orig.supersession_reason.as_deref(),
            Some("scope changed")
        );
        assert_eq!(
            loaded_orig.superseded_at.as_deref(),
            Some("2024-08-01T12:00:00Z")
        );
        ...
        assert_eq!(
            loaded_succ.supersession_reason.as_deref(),
            Some("scope changed")
        );
        assert_eq!(
            loaded_succ.superseded_at.as_deref(),
            Some("2024-08-01T12:00:00Z")
        );
    ...
    fn list_runs_includes_supersession_lineage() {
        ...
        let superseded = summaries
            .iter()
            .find(|s| s.run_id == "r_superseded")
            .unwrap();
    ...
    fn archive_metadata_roundtrip() {
        ...
        let meta = loaded
            .archive_metadata
            .expect("archive_metadata must be present");
    ...
    fn list_runs_include_archived_shows_all() {
        ...
        let runs = store
            .list_runs(20, None, None, true, false, None, false, false, false)
            .unwrap();
        assert!(
    ...
    fn list_runs_archived_only() {
        ...
        let runs = store
            .list_runs(20, None, None, false, true, None, false, false, false)
            .unwrap();
        assert!(
    ...
    fn list_runs_summary_carries_archive_fields() {
        ...
        assert_eq!(
            summary.archive_reason.as_deref(),
            Some("summary field test")
        );
    ...
    fn unarchive_metadata_roundtrip() {
        ...
        let meta = loaded
            .unarchive_metadata
            .expect("unarchive_metadata must be present");
    ...
    fn list_runs_summary_carries_unarchive_fields() {
        ...
        assert_eq!(
            summary.is_archived, None,
            "is_archived must be None for unarchived run"
        );
        assert_eq!(
            summary.unarchive_reason.as_deref(),
            Some("summary test unarchive")
        );
        assert_eq!(
            summary.unarchived_at.as_deref(),
            Some("2024-09-03T12:00:00Z")
        );
    }
    ...
    fn annotation_roundtrip() {
        ...
        assert_eq!(
            annotation.operator_note.as_deref(),
            Some("tracking regression")
        );
    }
    ...
    fn annotation_defaults_to_none() {
        ...
        assert!(
            loaded.annotation.is_none(),
            "annotation must default to None"
        );
    }
    ...
    fn list_runs_filter_by_label() {
        ...
        let auth_runs = store
            ...
            .unwrap();
        assert!(
            auth_runs.iter().any(|r| r.run_id == "r_lbl_auth"),
            "auth-labeled run must match"
        );
        assert!(
            !auth_runs.iter().any(|r| r.run_id == "r_lbl_infra"),
            "infra-labeled run must not match"
        );
        assert!(
            !auth_runs.iter().any(|r| r.run_id == "r_lbl_none"),
            "unlabeled run must not match"
        );

        // Filter by label=infra
        let infra_runs = store
            .list_runs(
                20,
                None,
                None,
            ...
            .unwrap();
        assert!(
            infra_runs.iter().any(|r| r.run_id == "r_lbl_infra"),
            "infra-labeled run must match"
        );
        assert!(
            !infra_runs.iter().any(|r| r.run_id == "r_lbl_auth"),
            "auth-labeled run must not match"
        );
    ...
    fn snoozed_run_excluded_from_default_list() {
        ...
        let runs = store
            ...
            .unwrap();
        assert!(
            !runs.iter().any(|r| r.run_id == "r_snz_excl"),
            "snoozed must be excluded by default"
        );
        assert!(
            runs.iter().any(|r| r.run_id == "r_snz_normal"),
            "normal must be included by default"
        );
    }
    ...
    fn snoozed_run_included_with_include_snoozed() {
        ...
        let runs = store
            .list_runs(20, None, None, false, false, None, false, true, false)
            .unwrap();
        assert!(
            runs.iter().any(|r| r.run_id == "r_snz_incl2"),
            "snoozed must be included"
        );
        assert!(
            runs.iter().any(|r| r.run_id == "r_snz_normal2"),
            "normal must also be included"
        );
    }
    ...
    fn snoozed_only_filter() {
        ...
        let runs = store
            .list_runs(20, None, None, false, false, None, false, false, true)
            .unwrap();
        assert!(
            runs.iter().any(|r| r.run_id == "r_snz_only2"),
            "snoozed must appear"
        );
        assert!(
            !runs.iter().any(|r| r.run_id == "r_snz_norm3"),
            "normal must not appear"
        );
    }
