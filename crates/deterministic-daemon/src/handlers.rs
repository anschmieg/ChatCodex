...
fn handle_patch_apply(
    ...
    let decision = deterministic_core::approval_policy::evaluate_patch(&p, &state.policy_profile);
...
fn handle_tests_run(
    ...
    let decision =
        deterministic_core::approval_policy::evaluate_test_run(&p, &state.policy_profile);
...
fn handle_runs_list(
    ...
    let label_filter_owned = p.label.as_deref().map(|l| l.trim().to_lowercase());
    ...
    if sort_by_priority {
        runs.sort_by(|a, b| b.priority.sort_key().cmp(&a.priority.sort_key()));
    }
    ...
    for run in &mut runs {
        let staleness =
            deterministic_core::run_staleness::derive_staleness(&run.updated_at, &reference_date);
...
fn handle_run_archive(
    ...
    let _ = store.append_audit_entry(
        ...
        Some(&serde_json::json!({ "reason": p.reason }).to_string()),
    );
...
fn handle_run_annotate(
    ...
    let note_updated =
        result.annotation.operator_note.is_some() || p.operator_note.as_deref() == Some("");
    let _ = store.append_audit_entry(
        &p.run_id,
        "run_annotated",
...
fn handle_run_set_dependencies(
    ...
    let result =
        deterministic_core::run_set_dependencies::set_dependencies(&p, &mut state, &known_ids)?;
    store.save_run(&state)?;
...
fn handle_runs_queue_overview(
    ...
    let result = RunQueueOverview {
        ...
        pinned_count,
        snoozed_count,
        overdue_count: if today.is_some() {
            Some(overdue_count)
        } else {
            None
        },
        archived_count: if include_archived {
            Some(archived_count)
        } else {
            None
        },
...
fn handle_queue_view_update(
    ...
    if let Some(ref name) = p.name {
        ...
        if trimmed.is_empty() {
            anyhow::bail!(
                "view name cannot be empty. Provide a non-empty name for the saved view."
            );
        }
        for v in views.values() {
    ...
    let result = UpdateQueueViewResult { view: view.clone() };
    Ok((serde_json::to_value(result)?, None))
}
...
use deterministic_protocol::{
    ...
    HybridWorkerPrepareResult, HybridWorkerRun, HybridWorkerStartParams, HybridWorkerStartResult,
    HybridWorkerStatus,
};
...
fn handle_hybrid_worker_prepare(
    ...
    let now = chrono::Utc::now().to_rfc3339();
    let profile = ensure_hybrid_enabled(hybrid_config)?;
    ...
    let result = HybridWorkerPrepareResult {
        ...
        status: HybridWorkerStatus::Prepared,
        recommended_next_action:
            "Worker run prepared. Call hybrid_start_worker_run to begin LLM execution.".to_string(),
...
fn handle_hybrid_worker_start(
    ...
    if worker.status != HybridWorkerStatus::Prepared {
        anyhow::bail!(
            "worker run '{}' cannot be started from status '{}'",
            worker.worker_run_id,
            worker.status.as_str()
        );
    }

    // Record the start time before making the LLM call.
    let started_at = chrono::Utc::now().to_rfc3339();
    worker.status = HybridWorkerStatus::Running;
    worker.started_at = Some(started_at.clone());
    worker.updated_at = started_at.clone();
    store.save_worker_run(&worker)?;

    // Call the LLM provider on a blocking thread so we never block the async runtime.
    let rt = tokio::runtime::Handle::current();
    let call_result = rt.block_on(async {
        tokio::task::spawn_blocking({
            let profile = profile.clone();
            let prompt = worker.prompt.clone();
            move || {
                let system_prompt =
                    crate::provider_client::worker_system_prompt();
                crate::provider_client::call_worker_provider_sync(
                    &profile,
                    &system_prompt,
                    &prompt,
                )
    ...
    worker.updated_at = completed_at.clone();
    worker.completed_at = Some(completed_at);

    match call_result {
        Ok((summary, edits)) => {
            worker.status = HybridWorkerStatus::Succeeded;
            worker.summary = summary;
            worker.proposed_edits = Some(edits);
            store.save_worker_run(&worker)?;

            let result = HybridWorkerStartResult {
                worker_run_id: worker.worker_run_id.clone(),
                status: HybridWorkerStatus::Succeeded,
                summary: worker.summary.clone(),
                proposed_edits: worker.proposed_edits.clone(),
                failure_message: None,
                recommended_next_action: "Worker completed. Review proposed_edits and apply"
                    .to_string(),
                recommended_tool: "apply_patch".to_string(),
            };
            Ok((serde_json::to_value(result)?, None))
        }
        Err(e) => {
            worker.status = HybridWorkerStatus::Failed;
            worker.failure_message = Some(e.to_string());
            store.save_worker_run(&worker)?;

            let result = HybridWorkerStartResult {
                worker_run_id: worker.worker_run_id.clone(),
                status: HybridWorkerStatus::Failed,
                summary: None,
                proposed_edits: None,
                failure_message: Some(e.to_string()),
                recommended_next_action: format!(
                    "Worker failed: {e}. Use hybrid_get_worker_run for details"
                ),
...
fn handle_hybrid_worker_cancel(
    ...
    let mut worker = store
        ...
        .ok_or_else(|| anyhow::anyhow!("unknown worker run: {}", p.worker_run_id))?;
    let now = chrono::Utc::now().to_rfc3339();
    store.mark_worker_cancel_requested(&worker.worker_run_id)?;
    worker.cancel_requested = true;
    worker.updated_at = now.clone();
    if matches!(
        worker.status,
        HybridWorkerStatus::Prepared | HybridWorkerStatus::Running
    ) {
        worker.status = HybridWorkerStatus::Cancelled;
...
fn ensure_hybrid_enabled(
    ...
    if !hybrid_config.is_enabled() {
        anyhow::bail!("hybrid mode is disabled");
    }

    let profile = hybrid_config
        .get_default_profile()
        .ok_or_else(|| anyhow::anyhow!("hybrid mode has no default provider profile"))?;
    profile
        .validate()
        .map_err(|e| anyhow::anyhow!("invalid hybrid provider profile: {e}"))?;
    Ok(profile)
}
...
mod tests {
    ...
    fn method_registry_includes_run_finalize() {
        ...
        assert_eq!(
            Method::parse_method("run.finalize"),
            Some(Method::RunFinalize)
        );
    }
    ...
    fn run_reopen_exposes_metadata_in_run_get() {
        ...
        let reopen_params =
            serde_json::json!({ "runId": "r_ro_get", "reason": "check metadata in get" });
    ...
    fn run_supersede_audit_trail_appended() {
        ...
        assert!(
            orig_superseded_event.is_some(),
            "run_superseded audit entry missing for original"
        );

        let succ_audit = store
            .get_audit_entries(&result.successor_run_id, 50)
            .unwrap();
        let succ_created_event = succ_audit
            .iter()
            .find(|e| e.event_kind == "run_created_from_supersession");
        assert!(
            succ_created_event.is_some(),
            "run_created_from_supersession audit entry missing for successor"
        );
    }
    ...
    fn run_supersede_lineage_visible_in_run_get() {
        ...
        assert_eq!(
            succ_get.supersedes_run_id.as_deref(),
            Some("r_sup_get_orig")
        );
    }
    ...
    fn run_archive_completed_run_succeeds() {
        ...
        let meta = loaded
            .archive_metadata
            .expect("archive_metadata must be persisted");
    ...
    fn run_archive_audit_trail_appended() {
        ...
        assert!(
            archived_event.is_some(),
            "run_archived audit entry must be present"
        );
    }
    ...
    fn run_archive_visible_in_run_get() {
        ...
        let get_result: RunGetResult = serde_json::from_value(val).unwrap();
        assert!(
            get_result.archive_metadata.is_some(),
            "archive_metadata must be visible in run.get"
        );
    ...
    fn run_archive_archived_only_filter() {
        ...
        let arch_params =
            serde_json::json!({ "runId": "r_ao_arch", "reason": "archived_only test" });
        dispatch(Method::RunArchive, arch_params, &store).unwrap();
    ...
    fn run_unarchive_completed_run_succeeds() {
        ...
        // Now unarchive.
        let params =
            serde_json::json!({ "runId": "r_unarch_c", "reason": "restoring for inspection" });
        ...
        let meta = loaded
            .unarchive_metadata
            .expect("unarchive_metadata must be persisted");
    ...
    fn run_unarchive_failed_run_succeeds() {
        ...
        let params =
            serde_json::json!({ "runId": "r_unarch_f", "reason": "reviewing failed build" });
    ...
    fn run_unarchive_rejected_for_non_archived_run() {
        ...
        assert!(
            err.to_string().contains("cannot be unarchived")
    ...
    fn run_unarchive_audit_trail_appended() {
        ...
        assert!(
            has_unarchive_entry,
            "run_unarchived audit entry must be appended"
        );
    }
    ...
    fn run_unarchive_restores_to_default_list() {
        ...
        assert!(
            !list.runs.iter().any(|r| r.run_id == "r_restore"),
            "archived run must not appear in default list"
        );
        ...
        assert!(
            list.runs.iter().any(|r| r.run_id == "r_restore"),
            "unarchived run must appear in default list"
        );
    }
    ...
    fn run_unarchive_excluded_from_archived_only_filter() {
        ...
        let (val, _) = dispatch(
            Method::RunsList,
            serde_json::json!({ "archivedOnly": true }),
            &store,
        )
        .unwrap();
        let list: RunsListResult = serde_json::from_value(val).unwrap();
        assert!(
            !list.runs.iter().any(|r| r.run_id == "r_ao_unarch"),
            "unarchived run must NOT appear with archivedOnly=true"
        );
    }
    ...
    fn run_unarchive_visible_in_run_get() {
        ...
        assert!(
            result.unarchive_metadata.is_some(),
            "unarchive_metadata must be visible in run.get"
        );
        assert!(
            result.archive_metadata.is_some(),
            "archive_metadata must still be present in run.get"
        );
    }
    ...
    fn run_unarchive_persistence_roundtrip() {
        ...
        let unarch_params =
            serde_json::json!({ "runId": "r_unarch_persist", "reason": "persistence test" });
        dispatch(Method::RunUnarchive, unarch_params, &store).unwrap();

        let loaded = store.get_run("r_unarch_persist").unwrap().unwrap();
        let meta = loaded
            .unarchive_metadata
            .expect("unarchive_metadata must persist");
    ...
    fn run_annotate_visible_in_run_get() {
        ...
        let annotation = result
            .annotation
            .expect("annotation must appear in run.get");
    ...
    fn run_snooze_snoozed_only_filter() {
        ...
        let result: RunsListResult = serde_json::from_value(val).unwrap();
        assert!(
            result.runs.iter().any(|r| r.run_id == "r_snz_only_a"),
            "snoozed run must appear"
        );
        assert!(
            !result.runs.iter().any(|r| r.run_id == "r_snz_only_b"),
            "non-snoozed run must not appear"
        );
    }
    ...
    fn run_unsnooze_persists_to_store() {
        ...
        assert!(
            loaded.snooze_metadata.is_none(),
            "snooze_metadata must be cleared after unsnooze"
        );
    }
    ...
    fn run_unsnooze_appends_audit_entry() {
        ...
        assert!(
            entry.is_some(),
            "run_unsnoozed audit entry must be appended"
        );
    }
    ...
    fn run_unsnooze_restores_to_default_list() {
        ...
        let (val, _) =
            dispatch(Method::RunsList, serde_json::json!({"limit": 50}), &store).unwrap();
        let result: RunsListResult = serde_json::from_value(val).unwrap();
        ...
        let result: RunsListResult = serde_json::from_value(val).unwrap();
        assert!(
            result.runs.iter().any(|r| r.run_id == "r_unsnz_list"),
            "unsnoozed run must appear in default list"
        );
    }
    ...
    fn run_set_priority_appends_audit_entry() {
        ...
        assert!(
            entry.is_some(),
            "run_priority_set audit entry must be appended"
        );
    }
    ...
    fn run_set_priority_summary_carries_priority() {
        ...
        let summary = result
            .runs
            .iter()
            .find(|r| r.run_id == "r_prio_summary")
            .unwrap();
    ...
    fn run_set_due_date_invalid_format_rejected() {
        ...
        assert!(
            msg.contains("10 characters")
                || msg.contains("non-digit")
                || msg.contains("separators"),
    ...
    fn runs_list_filter_by_due_on_or_before() {
        ...
        let ids: Vec<&str> = result.runs.iter().map(|r| r.run_id.as_str()).collect();
        assert!(
            ids.contains(&"r_dd_flt1"),
            "should include r_dd_flt1 (2026-01-15 ≤ threshold)"
        );
        assert!(
            !ids.contains(&"r_dd_flt2"),
            "r_dd_flt2 (2026-06-30) exceeds threshold"
        );
    ...
    fn runs_list_sort_by_due_date() {
        ...
        assert!(
            pos1 < pos4,
            "run with a date should sort before run with no date"
        );
    }
    ...
    fn runs_list_shows_is_blocking_and_blocking_run_count() {
        ...
        assert_eq!(
            summary_a.blocking_reason,
            Some("blocking 2 run(s)".to_string())
        );
    ...
    fn run_get_includes_blocker_impact_fields() {
        ...
        assert_eq!(
            ...
            Some("blocking 1 run(s)".to_string())
        );
    }
    ...
    fn queue_create_list_get_view() {
        ...
        assert!(
            found.is_some(),
            "Expected to find 'test-view' in views list"
        );
